		...
		if i>#self.CurrentGuide["steps"] or (delta>0 and self.CurrentStep.finish) then
			if self.CurrentGuide['next'] then
				...
				if self.CurrentStep.finish then
					...
					self.CurrentStep.goals={ [1]={ num=1, action="", text="На этом гайд подошёл к концу", parentStep=self.CurrentStep } }
...
function me:GoalOnClick(goalframe,button)
	...
	if button=="LeftButton" then
		...
		elseif goal.questid then
			...
			self:Print("Квест \""..goal.quest.."\" (#"..tostring(goal.questid).."): для уровня "..tostring(goal.parentStep.level)..", до уровня "..tostring(max))
			local mentioned = me:GetMentionedFollowups(goal.questid)
			if #mentioned>1 then
				local s=""
				for i=2,#mentioned do
					if #s>0 then s=s.."\n" end
					s=s.."\""..(me:GetQuestData(mentioned[i][1]) or "?").."\" (#"..tostring(mentioned[i][1])..") для уровня "..mentioned[i][2]
				end
				self:Print("Продолжение:\n"..s)
			else
				self:Print("Нет продолжения")
			end
		end
	else
		if self.recentlyCompletedGoals[goal] then
			...
			if goal.quest and IsShiftKeyDown() then
				...
				self:Print("Квест помечен '"..goal.quest.."'"..(goal.questid and " (#"..goal.questid..")" or "").." как невыполненный.")
			else
				self:Print("Шаг помечен как невыполненный.")
			end
		else
			--self.recentlyCompletedGoals[goal]=true
			...
			if goal.quest and IsShiftKeyDown() then
				...
				self:Print("Квест помечен '"..goal.quest.."'"..(goal.questid and " (#"..goal.questid..")" or "").." как выполненный.")
			end
		end
		self.pause=nil
		self.LastSkip=1
